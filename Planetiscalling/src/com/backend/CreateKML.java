...
public class CreateKML {
		...
		try {
			if (!"".equals(UtilityWeather.getInstance().getWeather().htmlData())) {
				...
				for (String key : dataline.getMapData().keySet()) {
					try {
						writer.write("<Folder><name>" + key + " distance </name>" + "<Placemark> "
								+ "<styleUrl>#msn_ylw-pushpin</styleUrl>" + " <Style>" + "  <LineStyle> " + "   <color>"
								+ dataline.getColor(key) + "</color>" + "<width>2</width> " + "  </LineStyle>"
								+ " </Style>" + "<LineString><extrude>1</extrude>" + "<tessellate>1</tessellate>"
	...
	public static String createHeader(int total) {
		return "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + "<kml xmlns=\"http://earth.google.com/kml/2.2\">"
				...
				+ "<name>KML file create by PlanetIsCalling</name>" + "<description>" + total
				+ " placemarks found!</description>";
	}

	static public String buildCityPlaceMark(City city) {

		String description = buildCityDescriptionLink(city);
		String icone = "<Style id=\"silo\"><IconStyle><Icon><href>" + ICON_MAP.get(city.getCapital())
				+ "</href></Icon></IconStyle></Style>";

		return "<Placemark><name>" + city.getCityName() + "</name>\n" + "<description><![CDATA[" + description
				+ "]]></description>\n" + icone + "<Point><coordinates>" + city.getCoordinates()
				+ "</coordinates></Point>\n" + "</Placemark>\n";
	}
	...
	static public String buildCityDescriptionLink(City city) {
		...
		description += "<div>City Name: "
				+ Util.createHref(city.getCityName(),
						city.getCityName() + " " + admin + " " + city.getCountry() + " wikipedia", 0)
				+ " ("
				+ Util.createHref("Weather", city.getCityName() + " " + admin + " " + city.getCountry() + " weather", 0)
				+ ")</div>";
		if (!city.getCityAscii().equals(city.getCityName())) {
			description += "<div>City Acsii: " + city.getCityAscii() + "</div>";
		}
		if (!"".equals(admin)) {
			description += "<div>" + stateName + ": " + Util.createHref(city.getAdminName(),
					city.getAdminName() + " " + city.getCountry() + " wikipedia", 0) + "</div>";
		}

		description += "<div>Country: " + Util.createHref(city.getCountry(), city.getCountry() + " wikipedia", 0)
				+ "</div>";
	...
	static public String buildLandmarkDescriptionPlane(Landmark landmark) {
		...
		// Util.createHref(landmark.getGeoName(),landmark.getGeoName()+"
		// "+landmark.getAdmin()+" Canada wikipedia", 0)

		description += "<div>Name: " + Util.createHref(landmark.getGeoName(),
				landmark.getGeoName() + " " + landmark.getAdmin() + " Canada wikipedia", 0) + "</div>";
	...
	static public String buildLandmarkPlaceMark(Landmark landmark, String visibitlity) {
		...
		return "<Placemark><name>" + landmark.getGeoName() + "</name>\n" + "<description><![CDATA[" + description
				+ "]]></description><visibility>" + visibitlity + "</visibility>\n" + icone + "<Point><coordinates>"
	...
	static public String buildVorPlaceMark(Vor vor) {
		...
		icone = "<Style id=\"silo\"><IconStyle><Icon><href>" + ICON_MAP.get("vor")
				+ "</href></Icon></IconStyle></Style>";

		return "<Placemark><name>VOR " + vor.getIdent() + "</name>\n" + "<description><![CDATA[" + description
				+ "]]></description>\n" + icone + "<Point><coordinates>" + vor.getCoordinates()
				+ "</coordinates></Point>\n" + "</Placemark>\n";
	}
	...
	static public String buildVorDescription(Vor vor) {
		...
		description += "<div>Altitude: " + vor.getAltitude() + " ft (" + ((int) Math.round(vor.getAltitude() / 3.28084))
				+ " m)</div>";
	 ...
	 * String description = "";
	 * 
	 * description += "<div>Ident: "+legPoint.getIdent()+"</div>"; description +=
	 * "<div>Name: "+legPoint.getName()+"</div>"; description +=
	 * "<div>Frequency: "+Util.formatVorFrequency(vor.getFrequency())+" MHz</div>";
	 * description += "<div>Range: "+vor.getRange()+"nm</div>"; description +=
	 * "<div>Mag. Var.: "+Util.formatMagvar(vor.getMagVar())+"</div>"; description
	 * +=
	 * "<div>Altitude: "+vor.getAltitude()+" ft ("+((int)Math.round(vor.getAltitude(
	 * )/3.28084))+" m)</div>"; description += "<div>Type: "+vor.getType()+"</div>";
	 * description += "<div>DME Only: "+(vor.getDmeOnly()==1?"Yes":"No")+"</div>";
	 * description += "<div>Region: "+vor.getRegion()+"</div>"; description +=
	 * "<div>GPS: "+Util.formatGPS(vor.getLaty()+","+vor.getLonx())+"</div>";
	 * description =
	 * "<div style=\"width: 300px; font-size: 12px;\">"+description+"</div>";
	 * 
	 * 
	 * return description;
	 * 
	...
	static public String buildNdbPlaceMark(Ndb ndb) {
		...
		icone = "<Style id=\"silo\"><IconStyle><Icon><href>" + ICON_MAP.get("ndb")
				+ "</href></Icon></IconStyle></Style>";

		return "<Placemark><name>NDB " + ndb.getIdent() + "</name>\n" + "<description><![CDATA[" + description
				+ "]]></description>\n" + icone + "<Point><coordinates>" + ndb.getCoordinates()
				+ "</coordinates></Point>\n" + "</Placemark>\n";
	}
	...
	static public String buildNdbDescription(Ndb ndb) {
		...
		description += "<div>Altitude: " + ((int) Math.round(ndb.getAltitude() * 3.28084)) + "ft (" + ndb.getAltitude()
				+ "m)</div>";
	...
	static public String buildMountainPlaceMark(Mountain mountain) {
		...
		icone = "<Style id=\"silo\"><IconStyle><Icon><href>" + ICON_MAP.get(typeIcone)
				+ "</href></Icon></IconStyle></Style>";

		return "<Placemark><name>" + mountain.getName() + "</name>\n" + "<description><![CDATA[" + description
				+ "]]></description>\n" + icone + "<Point><coordinates>" + coordinates + "</coordinates></Point>\n"
				+ "</Placemark>\n";
	}
	...
	static public String buildMountainDescription(Mountain mountain) {
		...
		description += "<div>Mountain Name: " + Util
				.createHref(mountain.getName(), mountain.getName() + " " + mountain.getCountry() + " wikipedia", 0)
				+ " ("
				+ Util.createHref("Weather",
						mountain.getName() + " " + mountain.getName() + " " + mountain.getCountry() + " weather", 0)
				+ ")</div>";
		if (!"no".equals(mountain.getAlt_name())) {
			description += "<div>Alternate Name:" + Util.createHref(mountain.getAlt_name(),
					mountain.getAlt_name() + " " + mountain.getCountry() + " wikipedia", 0) + "</div>";
		}
		description += "<div>Elevation: " + mountain.getElevation() + "m" + " ("
				+ Math.round(mountain.getElevation() * 3.28084) + "ft)</div>";
		if (mountain.getProminence() > 0) {
			description += "<div>Prominence: " + mountain.getProminence() + "m" + " ("
					+ Math.round(mountain.getProminence() * 3.28084) + "ft)"
					+ Util.createHref(" (?)", "prominence wikipedia", 0) + " </div>";
		}
	...
	static public String buildMountainDescriptionNoWeather(Mountain mountain) {
		...
		description += "<div>Mountain Name: " + Util.createHref(mountain.getName(),
				mountain.getName() + " " + mountain.getCountry() + " wikipedia", 0);
		if (!"no".equals(mountain.getAlt_name())) {
			description += "<div>Alternate Name:" + Util.createHref(mountain.getAlt_name(),
					mountain.getAlt_name() + " " + mountain.getCountry() + " wikipedia", 0) + "</div>";
		}
	...
	static public String buildAirportPlaceMark(Airport airport) {
		return "<Placemark><name>" + airport.getIdent() + " " + airport.getName() + "</name>\n"
				...
				+ "<Style id=\"airport\"><IconStyle><Icon><href>" + ICON_MAP.get("airport")
				+ "</href></Icon></IconStyle></Style>" + "<Point><coordinates>" + airport.getCoordinates()
				+ "</coordinates></Point>\n" + "</Placemark>\n";
	}
