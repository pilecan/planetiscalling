...
public class Result implements Info {
	...
	public JPanel getFlightPlanFormPanel() {
		...
		// label.setToolTipText(this.flightplan.getDepartureName() + "/" +
		// this.flightplan.getDestinationName());
		panel = new JPanel();
		...
		for (int i = 0; i < legPoints.size(); i++) {
			...
			if (i < legPoints.size() - 1) {
				...
				if ("1".equals(legPoints.get(i).getVisible())) {
					distance = Geoinfo.distance(legPoints.get(i).getLaty(), legPoints.get(i).getLonx(),
							legPoints.get(i + 1).getLaty(), legPoints.get(i + 1).getLonx(), 'N');
					distanceCumul += distance;
					builder = Utility.getInstance().buildLine(legPoints.get(i).getId(), Math.round(distance),
							Math.round(distanceCumul));
				}
			} else {
				builder = Utility.getInstance().buildLine(legPoints.get(legPoints.size() - 1).getId(), 0,
						Math.round(distanceCumul));
	...
	public void getCityListModel() {
		...
		for (City city : selectedCities.values()) {
			builder = Utility.getInstance().buildLine(city.getCityName(),
					(!city.getCityName().equals(city.getCityAscii()) ? city.getCityAscii() : city.getAdminName()),
					(city.getPopulation() > 0 ? city.getPopulation() : "Unknow Pop"));
			((DefaultListModel) listModel).addElement(builder.toString());
		}
	...
	public void getMountainListModel() {
		...
		for (Mountain mountain : selectedMountains.values()) {
			...
			builder = Utility.getInstance().buildLine(mountain.getName(), mountain.getCountry(),
	...
	public String panelCity(String line) {
		...
		for (City city : selectedCities.values()) {
			if (city.getCityName().equals(line)) {
				htmlString += CreateKML.buildCityDescriptionLink(city).replaceAll("12px", "10px").replaceAll("\\|",
						"<br>");
	...
	public String panelMountain(String line) {
		...
		for (Mountain mountain : selectedMountains.values()) {
			if (mountain.getName().equals(line)) {
				htmlString = CreateKML.buildMountainDescriptionNoWeather(mountain).replaceAll("12px", "10px")
						.replaceAll("\\|", "<br>");
				try {
	...
	public String panelLandmark(String line) {
		...
		for (Landmark landmark : selectedLandmarks.values()) {
			if (landmark.getGeoName().equals(line)) {
				htmlString = CreateKML.buildLandmarkDescriptionPlane(landmark).replaceAll("12px", "10px")
						.replaceAll("\\|", "<br>");
				try {
	...
	public void showPanelInfo(String content) {
		...
		try {
			...
			// ->"+content.substring(content.indexOf("<!--")+4,content.indexOf("-->")));
		...
		try {
			...
			jEditorPane.setText("<html><body style='font-weight: bold; color: #" + color
					+ "; background-position: 200px 0; background-image: url(" + urlString
					+ "); background-repeat: no-repeat;'>" + content + "</body></html>");
